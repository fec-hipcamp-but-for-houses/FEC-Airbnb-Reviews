...
const seedingMyMessages = () => {
  for (let i = 1; i <= 100; i += 1) {
    ...
    const reviewScore = faker.finance.amount(0, 5, 2);
    const realDate = moment(date).format('YYYY-MM-DD');
    db.query(`INSERT INTO Messages(message, user_id, username, profile_picture, listing, date, accuracy, communication, cleanliness, check_in, value, location) VALUES('${message}', ${userId}, '${username}', '${profilePic}', ${listing}, '${realDate}', ${reviewScore}, ${reviewScore}, ${reviewScore}, ${reviewScore}, ${reviewScore}, ${reviewScore});`, (err, data) => {
      if (err) {
        console.log('ERROR GETTING DATA INSERTED', err);
      } else {
        console.log('SUCCESSFUL ENTRY', data);
